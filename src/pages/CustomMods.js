...
const CarModelRequest = () => {
  ...
  const [canSubmit, setCanSubmit] = useState(true); // ✅ controls form visibility
  ...
  const handleChange = (e) => {
    ...
    if (name === "reference_files") {
      ...
      if (oversizedFiles.length > 0) {
        alert(
          `Each file must be less than ${MAX_FILE_SIZE_MB}MB. Please reselect files.`
        );
        e.target.value = "";
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    for (let file of formData.reference_files) {
      if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
        alert(
          `File "${file.name}" exceeds ${MAX_FILE_SIZE_MB}MB limit. Please remove it.`
        );
        setIsSubmitted(false);
        return;
      }
    ...
    try {
      ...
      const response = await axios.post(
        `${databaseApiUrl}gta_model_request.php`,
        formPayload,
        {
      ...
      if (data.status_id === "1") {
        ...
        // ✅ Save cookie (24 hours restriction)
  ...
  return (
    <div className="container my-5">
        ...
        >
          <div
            style={{
              ...
              textAlign: "center",
              maxWidth: "420px",
              fontFamily: "'Orbitron', sans-serif", // gaming-style font (add via Google Fonts)
            }}
          >
            <h5 style={{ marginBottom: "15px", fontSize: "22px", textTransform: "uppercase" }}>
              Request Submitted
            </h5>
            <p style={{ color: "#ddd", fontSize: "16px", lineHeight: "1.5" }}>
              We will be in touch with you within{" "}
              <strong style={{ color: "#ff3333" }}>12 hours</strong> through your email.
            </p>
            <button
              onClick={() => {
                setShowPopup(false);
      ...
      {/* ✅ Form */}
      {canSubmit ? (<div className="row justify-content-center">
        <div className="col-md-10 col-lg-8">
          <div className="card shadow text-light" style={{ backgroundColor: '#131111' }}>
            <div className="card-body p-4">
              <h4 className="text-center text-light mb-4">
                Here you can request the creation of your own model from us.
              </h4>
              <p className="text-center text-danger mt-5">
                Fill the requirements please!
              </p>
              ...
              <form onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                <div className="mb-3">
                  ...
                  <small className="text-light">
                    Upload up to {MAX_FILES} files ({MAX_FILE_SIZE_MB}MB each). For files over 40MB, upload to Google Drive and share the link.
                  </small>
                      ...
                      {formData.reference_files.map((file, idx) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              const updatedFiles =
                                formData.reference_files.filter(
                                  (_, i) => i !== idx
                                );
                              setFormData({
                                ...formData,
                                reference_files: updatedFiles,
                              });
                            }}
                          >
                            ❌
